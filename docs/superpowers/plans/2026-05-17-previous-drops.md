...
- **Fifth consumer (found during Task 9 verification, not in the original spec):** `src/app/club/[dropId]/page.tsx` gated the Bread Club selection window on raw `drop.status === "announced"`. Fixed to `effectiveDropStatus(drop, new Date()) === "announced"` so it agrees with `getMemberSelectionsForDrop` (same concept, same derivation). Future raw-`drop.status` consumers should likewise switch to effective status.
